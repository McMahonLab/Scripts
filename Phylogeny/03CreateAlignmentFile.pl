...
opendir(DIR, "PS_temp") || die "Can't open directory: PS_temp\n";

my @dirList = grep {-d "PS_temp/$_" && ! /^\.{1,2}$/} readdir(DIR);
...
for my $i (0 .. $arrayLen-1) {
  ...
  open(inputFile,'<', "PS_temp/$dirList[$i]/alignDir/concat.updated.1.fasta");
  select(outputFile);
